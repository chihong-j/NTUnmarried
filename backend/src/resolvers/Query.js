...
const Query = {
    async user(parent, { email }, { db }, info) {
        ...
        const { _id: id, name, gender, age, aboutMe, department, password, paired } = queryUser;
        if (!queryUser.images) return { id, email, name, gender, age, aboutMe, department, images: [], password, paired};
        const images = [];
        ...
        return { id, email, name, gender, age, aboutMe, department, images, password, paired};
    },
